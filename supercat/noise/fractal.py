...
def interpolant(t):
    """    
    Taken from https://github.com/pvigier/perlin-numpy/
    """
    return t*t*t*(t*(t*6 - 15) + 10)


def shape_for_fractal_noise(shape, res, octaves, lacunarity):
    shape = list(shape)
    for i, (s, r) in enumerate(zip(shape, res)):
        multiple_of = lacunarity**(octaves-1)*r
        if s % multiple_of != 0:
            shape[i] += multiple_of - (s % multiple_of)
    return tuple(shape)
...
def generate_fractal_noise_2d(
    ...
    # Make shape bigger if necessary
    fractal_shape = shape_for_fractal_noise(shape, res, octaves, lacunarity)

    noise = np.zeros(fractal_shape)
    frequency = 1
    amplitude = 1
    for _ in range(octaves):
        noise += amplitude * generate_perlin_noise_2d(
            fractal_shape, (frequency*res[0], frequency*res[1]), tileable, interpolant
        )
        frequency *= lacunarity
        amplitude *= persistence

    # Crop if necessary
    if fractal_shape != shape:
